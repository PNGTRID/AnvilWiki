...
站上线了,Google 在收录了。但访客来了你没东西卖——这一课把**广告位**开起来,访客看到广告,Google 就给你分钱。

## 先说时机:别一上线就开广告

这一课的开关操作随时能做,但**什么时候拨上去有讲究**:
...
- ☐ 自有域名(上线那一课买的;pages.dev 免费域名基本过不了审)
- ☐ 15 到 20 篇真实内容(不是空壳)
...
**被拒了怎么办**:看它给的原因,九成是「内容价值不足」——回去用产页那一课的套路再写 5 到 10 篇,过两周再申请,不影响后续。
...
| Microsoft Clarity | 不要 | 点击热力图、操作录屏 | 每周保鲜那一课有 10 分钟教程,配合每周复盘 |
...
2. 它给你一段 JS 代码,里面 `token="一串字母数字"`——把这串 token 填进环境变量 **`PUBLIC_CF_BEACON_TOKEN`**(填的位置和本课第二步的 4 个广告变量一样)
...
广告开了,但游戏攻略最怕过期——过期内容会掉排名、掉访客。最后一章:每周 30 分钟的保鲜节奏,让站一直赚钱。[去每周保鲜与增长](/zh/landing/docs/weekly-ops)
