...
- **本地可视化初始化/内容后台**(社区 issue #21):零终端初始化已有 Actions 表单通道、本地已有 `apply-template` 交互问答;「本地常驻后台(可视化写作/素材管理/对接 AI 生成/选栏目发布)」是独立产品量级的新形态,涉及本地服务与编辑器面——是否做、何时做,视 fork 用户真实反馈再评估。
