...
| `INDEXNOW_KEY`              | 兼容 | 旧站兼容项；新 fork 由 `.indexnow-key` 自动管理，无需配置 |
...
3. **主动推送收录**：
   - **Cloudflare Crawler Hints**：Cloudflare 控制台 → 你的域名 → Caching → Crawler Hints 打开。
   - **IndexNow 自动推送**：新 fork 在运行 `Initialize AnvilWiki` 或 `pnpm apply-template` 时会自动生成一次 `.indexnow-key`。这个文件随初始化提交进入仓库，之后 build 与 GitHub Actions 共用同一个 key；**不需要再配置 Cloudflare / GitHub 的 `INDEXNOW_KEY` 或 Actions `SITE_URL` 变量**。

#### IndexNow：初始化后零额外配置

`.indexnow-key` 是站点级公开所有权 key，不是 secret。IndexNow 本来就要求把同一个 key 以 `/<key>.txt` 公开在站点根路径。AnvilWiki 只在**明确的站点初始化阶段**生成它一次：

- GitHub：fork 后运行 **Initialize AnvilWiki**，初始化 PR 会包含 `.indexnow-key`。
- 本地：运行 `pnpm apply-template`，如果文件不存在会生成；重复运行会保留原 key。
- 普通 `git commit`、`pnpm build`、Cloudflare deploy、IndexNow workflow **都不会生成或轮换 key**。

`pnpm build` 的 postbuild 会读取 `.indexnow-key` 并生成 `dist/<key>.txt`。Cloudflare Pages 构建同时利用 `CF_PAGES_COMMIT_SHA` 生成 `dist/.well-known/anvilwiki-deploy.txt`，供自动任务确认**当前 commit 已经真正上线**后再读取生产 sitemap。

以后每次 `main` push 的 CI 成功后，`.github/workflows/indexnow.yml` 会：

1. checkout 触发 CI 的准确 commit；
2. 检测 `.indexnow-key`；未初始化的模板只 checkout 后跳过；
3. 从仓库里的 `SITE_URL` / `site.domain` 解析生产域名；
4. 等待生产部署标记匹配当前 commit；
5. 校验 `/<key>.txt` 已上线；
6. 读取生产 sitemap，向 IndexNow 批量提交；429 / 5xx 做短暂重试。

**存量站兼容**：旧版的 `INDEXNOW_KEY`（Cloudflare/wrangler、GitHub Actions variable、本地 `.env`）仍可继续使用。初始化发现旧 key 时会优先迁移同一个值到 `.indexnow-key`，不会偷偷换 key。若仓库文件与旧变量同时存在但值不同，build / submit 会直接失败，要求先处理冲突。

**key 轮换**：正常不需要轮换。确需轮换时，修改 `.indexnow-key` 并确保任何遗留 `INDEXNOW_KEY` 同步或删除，同时确认旧版曾提交的 `public/<old-key>.txt` 已从生产部署移除。不要在 build 或 workflow 中随机生成新 key。
...
旧版的「扫描 `public/*.txt` 自动认 key」不会恢复；第 21 轮审计已经证明它可能误捡 demo/过期 key。现在唯一的新站主路径是明确的 `.indexnow-key`，环境变量只保留兼容用途。
